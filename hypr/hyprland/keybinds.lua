...
create_bind(
    ...
    function(key)
        local normalised = normalise_keybind(key)
        local left = normalise_keybind("SUPER + SUPER_L")
        local right = normalise_keybind("SUPER + SUPER_R")
        return (normalised == left or normalised == right) and release or nil
    end
)
create_bind(
    { "SUPER + mouse:274", "SUPER + mouse:275", "SUPER + mouse:276", "SUPER + mouse:277" },
...
hl.bind("SUPER + SHIFT + mouse_left", hl.dsp.layout("swapcol r"))
hl.bind("SUPER + SHIFT + mouse_right", hl.dsp.layout("swapcol l"))
...
create_bind("SUPER + A", fn.toggle("grid"))
...
create_bind("SUPER + Z", hl.dsp.exec_cmd("zen-browser"))
...
create_bind("SUPER + B", hl.dsp.exec_cmd("brave-origin-nightly --refresh-platform-policy --force-device-scale-factor=1.25"))

-- Utilities
create_bind(vars.kbScreenshot, hl.dsp.exec_cmd("~/.local/bin/shot-focused-monitor"), locked)
create_bind(vars.kbScreenshotFreeze, hl.dsp.exec_cmd("~/.local/bin/shot-region"))
...
create_bind("CTRL + SHIFT + Space", hl.dsp.exec_cmd("handy --toggle-transcription"))
create_bind(
